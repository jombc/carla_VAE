...
class BenchmarkRunner(object):
        ...
        for experiment in experiment_suite.get_experiments()[int(start_experiment):]:
            ...
            for pose in experiment.poses[start_pose:]:
                for rep in range(start_rep, experiment.repetitions):
                    ...
                    print('======== !!!! ==========')
                    print(' Start Position %d End Position %d ', start_index, end_index)
                    ...
                    self._recording.write_measurements_results(experiment, rep, pose, reward_vec, control_vec)

                    if result > 0:
                        # logging.info('+++++ Target achieved in %f seconds! +++++', final_time)
    ...
    def _has_agent_collided(self, measurement, metrics_parameters, forward_speed):
    ...
    def _run_navigation_episode(
        ...
        while not fail and not success:
            ...
            control = agent.run_step(measurements, sensor_data, directions, target, episode_name)
            ...
            col_ped, col_veh, col_oth = self._has_agent_collided(measurements.player_measurements,
                                                                 metrics_parameters, measurements.player_measurements.forward_speed)
